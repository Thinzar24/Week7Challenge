...
public class HomeController {
    ...
    @Autowired
    WeeddayRepository weeddayRepository;


    @RequestMapping("/")
    public String fruitsDetails(Model model) {
        model.addAttribute("fruits", fruitRepository.findAll());
        model.addAttribute("weekdays", weeddayRepository.findAll());
        return "index";
    }
    ...
    public String processForm(@Valid Week weekday, BindingResult result, HttpServletRequest request, Model model) {
        if (result.hasErrors()) {
            return "weekform";
        }
        String search = request.getParameter("id");
        model.addAttribute("listResult",weeddayRepository.findByDaysIgnoreCase(search));
        return "weekform";
    }
